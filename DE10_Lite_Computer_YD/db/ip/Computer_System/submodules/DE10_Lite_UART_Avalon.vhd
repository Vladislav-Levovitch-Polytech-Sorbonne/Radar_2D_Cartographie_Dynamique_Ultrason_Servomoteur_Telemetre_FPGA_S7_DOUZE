...
entity DE10_Lite_UART_Avalon is
    port 
    (
        ...
        Read_n    : in  std_logic;  
        WriteData : in  std_logic_vector(31 downto 0); -- IO WRite + 0 Signal de chargement  -- IO WRite + 1 Donnee ASCii a transmettre
        Write_n   : in  std_logic;  
        ReadData  : out std_logic_vector(31 downto 0); -- IO ReaD ancien Rx_Data Lecture data transmise
...
architecture Behavioral of DE10_Lite_UART_Avalon is
    ...
    constant DIV_FACTOR : integer := 868; -- 434*2 Division pour une horloge de 50*2 MHz et un baud rate de 115200
    ...
    signal SIGNAL_Data_Rx_Out  : std_logic_vector(7 downto 0) := (others => '0');
    ...
    begin
        if Reset_n = '0' then
            ...
            if Write_n = '0' then
                case Address is
                    when "00" => -- Load
                        SIGNAL_Load_Save <= WriteData(0);
                    when "01" => -- Ascii
                        SIGNAL_Data_Tx   <= WriteData(7 downto 0);
                    when others =>
                        null;
                end case;
            elsif Read_n = '0' then
                case Address is
                    when "10" => -- Rx_Data
                        ReadData <= "000000000000000000000000" & SIGNAL_Data_Rx_Out;
                    when others =>
                        ReadData <= (others => '0');
                end case;
            end if;
                    ...
                    case SIGNAL_Etape_Lecture is
                        ...
                        when Check_Stop_Bit =>
                            ...
                            if Uart_Rx = '1' then
                                ...
                                SIGNAL_Data_Rx_Out   <= SIGNAL_Data_Rx; -- Copie uniquement si la trame est conforme, entier Start data Stop inclu
...
-- Code based on UART tx Yann DOUZE and gpt